...
chrome.runtime.onMessage.addListener(async function (message, sender) {
    ...
    const oldMatch = senderMustBeInAparty.exec(sender.tab.url);
    if (sender.tab && oldMatch) {
        switch (message.type) {
            case 'movePartyToNewVideo':
                const [, oldVideoTitle] = oldMatch;
                const queryOptions = {active: true, lastFocusedWindow: true};
                const [tab] = await chrome.tabs.query(queryOptions);
                if (tab.pendingUrl) {
                    const newVideoMustBeFrom = /^https:\/\/www3\.animeflv\.net\/ver\/(.*)$/;
                    const newMatch = newVideoMustBeFrom.exec(tab.pendingUrl);
                    if (newMatch) {
                        const [, newVideoTitle] = newMatch;
                        ...
                        console.log("no hubo newmatch: ", tab.pendingUrl)
                    }
        ...
        console.log("no hubo oldmatch: ", sender.tab.url)
    }
