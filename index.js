...
import(src).then(endpointsModule => {
    ...
    console.log("EJECUCION EN: ", window);

    if (window.frameElement === null && currentUrl.startsWith(verifyUrl)) {
        console.log("EJECUCION EN ANIMEFLV!: ", window)
        animeUrl = currentUrl;
        ...
        const createChatView = (type, roomId, username, exceptionalCase) => {
            ...
            $chatIframe.addEventListener("load", e => {
                ...
                if (exceptionalCase) {
                    switch (exceptionalCase.situation) {
                        case 'movingPartyToNewVideo':
                            window.history.replaceState(null, null, `?roomId=${roomId}`);
                            break;
                        default:
        ...
        if (searchParams.has('roomId')) {
            chrome.storage.sync.get('username', (data) => {
                ...
                if (!username) {
                    ...
                    const $modal = d.querySelector(".frame");
                    d.querySelector(".modalWindow button").addEventListener("click", () => {
            ...
            switch (msg.type) {
                ...
                case "updatePartyToNewVideo": {
                    ...
                    if ($videoOptionsContainer) {
                        ...
                        createChatView(userType, roomId, username, {
                            situation: 'movingPartyToNewVideo',
                            url,
                            videoProvider
                        });
        ...
        console.log("EJECUCION EN EL IFRAME!: ", window)
        let videos = [];

        try {
            const getVideos = setInterval(() => {
                console.log("intentando obtener videos")
                videos = d.querySelectorAll("video");
                if (videos.length > 0) {
                    console.log("se logro: ", videos)
                    clearInterval(getVideos)
                    videos.forEach((item) => {
            ...
            console.log("error al intentar ejecutar la query video en iframe: ", e)
        }
